/**
 * CompAgentSHAP - Live Mock SHAP Impact Calculator
 * 
 * This module calculates mock SHAP values based on the differences between
 * a comparable property and a subject property. In a production environment,
 * this would be replaced with actual SHAP value calculations from a backend API.
 */
...
/**
 * Calculate mock SHAP values based on property differences
 */
export function calculateShapValues(compProperty: CompProperty, subjectProperty: CompProperty): ShapData {
...
function calculateDistanceEffect(distance: number): number {
...
function calculateSqftEffect(sqftDiff: number): number {
  // Approximate value of $100 per sqft difference
  return -sqftDiff * 100;
}

function calculateBedroomEffect(bedDiff: number): number {
  // Value adjustment per bedroom
  return -bedDiff * 5000;
}

function calculateBathroomEffect(bathDiff: number): number {
  // Value adjustment per bathroom
  return -bathDiff * 7500;
}

function calculateYearEffect(yearDiff: number): number {
  // Newer homes are worth more, approximately $500 per year
  return -yearDiff * 500;
}

function calculateSaleDateEffect(compDate: string, subjectDate: string): number {
